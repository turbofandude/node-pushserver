...
var initialize = _.once(function () {
    ...
    var pushAssociationSchema = new db.Schema({ //Newer versions may require mongoose.Schema
        user: {
            type: 'String',
            required: true
        },
    ...
    PushAssociation = db.model('PushAssociation', pushAssociationSchema); //Newer versions may require mongoose.model
...
var removeDeviceFromUser = function(user, token) {
    PushAssociation.remove({user: user, token: token}, function(err) {
        if(err) console.log(err);
    });
};
